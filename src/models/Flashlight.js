...
function addWildCards(q, before, after) {
  const query = q
    .replace(/\s+/, ' ')
    .trim()
    .replace(new RegExp('[^ A-Za-z0-9' + unicodeClasses() + ']'), '');
  if (query) {
    const b = before ? '*' : '';
    const a = after ? '*' : '';
    return b + query.replace(' ', b + ' ' + a) + a;
  }
  return '';
}
...
module.exports = class Flashlight {
  ...
  search(query, ...resources) {
    ...
    (all ? Object.keys(Config.resources) : resources).forEach((r) => {
      ...
      (personalAllowed ? [r, 'personal_' + r] : [r]).forEach((resource) => {
        if (resources.indexOf(resource) > -1 || Current.permissions[resource].read) {
          promises.push(new Promise((resolve, reject) => {
            if (resource !== 'users' && !Current.permissions[resource].read) {
              reject('No permission on resource ' + resource);
              return;
            }
            let results = false;
            const finalQuery = Object.assign({
              index: Current.organization.key,
              type: resource
            }, query);
            if (finalQuery.q) {
              finalQuery.body = {
            ...
            if (resource.indexOf('personal_') === 0) {
              ...
              finalQuery.body.query = {
                bool: {
                  ...
                  filter: {
                    term: {
                      'creator.keyword': Current.user.uid
                    }
                  }
                }
              };
  ...
  fuzzy(q, ...resources) {
    return this.search({ q: addWildCards(q, true, true) }, ...resources);
  }

  suggest(q, ...resources) {
    return this.search({ q: addWildCards(q, false, true) }, ...resources);
  }
